...
- 在 SillyTavern 扩展设置中显示主题、全部保留版本、说明和预览图。
- 用户批准的版本默认展示；其他已上传版本保留在默认收起的“其他版本”中。
- 主题按日间与黑夜分组，版本显示可读名称、批准/推荐标记和简短更新记录。
...
2. 点击“检查更新”。
3. 默认选择已批准的推荐版本；如需试用其他版本，展开主题卡片中的“其他版本”。
...
当前目录已经发布33个主题 JSON，插件按内部名称归为32个主题。所有版本都会保留；已批准版本直接展示，其他版本默认收起但仍可安装。
...
- 已安装主题：点击“切换”即可直接启用。
- 远程主题有新版本：新版本会进入目录，但只有明确批准后才默认展示；未批准时位于“其他版本”。
- 使用历史或测试版本：展开“其他版本”，选择后点击“安装此版本并切换”；想回到批准版本时在上方“展示版本”中重新选择。
...
- `versions` 数组；每个版本包含 `version`、`version_name`、`changelog`、`theme_url`、`approved` 和对主题 JSON 原始字节计算得到的 `sha256`

更新主题时必须追加并保留版本记录，不能因为版本未批准就从目录删除。`approved: true` 表示用户已同意、默认展示；`approved: false` 表示仍可安装，但默认收进“其他版本”。`default_version` 只能指向已批准版本；最新版本不会自动获得批准。订阅器仍兼容 schema v1/v2：旧目录缺少 `approved` 时，`status: complete` 视为已批准，`status: test` 视为未批准，并继续以 SHA-256 判断实际内容。
...
`scripts/build-online-catalog.mjs` 用于把维护者本机的 `C:\1234` 完成品和 `C:\aaaa` 测试主题合并为在线目录。所有 JSON 都上传，旧目录中的固定提交链接会继续保留，内部名称重复的文件归入同一主题的历史版本列表。`C:\1234` 默认记为已批准，`C:\aaaa` 默认记为未批准；可在 `catalog-metadata.json` 为单个源文件显式设置 `approved` 覆盖默认值。测试目录中的同名文件仍可成为最新版，但不会因此自动展示或替换批准版本。
