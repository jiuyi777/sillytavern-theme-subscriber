...
- 页面右下角常驻独立救援按钮：无论当前主题怎样美化普通按钮，都可以强制返回安装前的上一个主题。
- 救援按钮旁的“删”会先切换到其他可用主题，再调用酒馆原生接口删除当前坏主题，避免删除后仍卡在坏CSS里。
- 电脑端还可以按 `Ctrl+Alt+Shift+R` 强制返回；按钮本身使用隔离 Shadow DOM、最高层级和自恢复样式，尽量不受主题 CSS 影响。
...
- 某个主题让页面按钮点不动：点击右下角红色“↩ 返回上个主题”；需要删除时点击旁边“删”并确认。电脑上按钮也被意外遮挡时按 `Ctrl+Alt+Shift+R`。
